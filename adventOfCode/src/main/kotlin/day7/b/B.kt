...
fun main() {
    var input = "32T3K 765\n" +
            ...
            "QQQJA 483"
    var ret = calc(input)
    require(ret == 5905L) { "Expected 5905, got ${ret}" }
...
object cardComparator : Comparator<Pair<List<Card>, Int>> {
    override fun compare(a: Pair<List<Card>, Int>, b: Pair<List<Card>, Int>): Int {
        val valA = determineCardsMaxValue(a.first)
        val valB = determineCardsMaxValue(b.first)
    ...
    private fun determineCardsMaxValue(cards: List<Card>): Int {
        return generateAllJokerCombinations(cards)
                .map { it.sortedBy { card -> card.value } }
                .distinct()
                .maxOfOrNull { determineCardsValue(it) }!!
    }

    private fun generateAllJokerCombinations(cards: List<Card>): List<List<Card>> {
        val nonJokerCards = cards.filter { it.value != 1 }
        return addAllJokerPossibilities(mutableListOf(nonJokerCards))
    }

    private fun addAllJokerPossibilities(combinations: List<List<Card>>): List<List<Card>> {
        return if (combinations[0].size < 5) {
            combinations.flatMap { cards ->
                addAllJokerPossibilities(IntRange(1,14).map { cards + Card(it) })
            }
        } else {
            combinations
        }
...
fun determineCardsValue(cards: List<Card>): Int = when {
    ...
    cards.groupBy { it.value }.mapValues { it.value.count() }.count { it.value == 2 } == 2 -> 3 // Double pair
    cards.groupBy { it.value }.mapValues { it.value.count() }.count { it.value == 2 } == 1 -> 2 // One pair
    else -> 1
}
